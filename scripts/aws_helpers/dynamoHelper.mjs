 ...
 * @property {string|undefined} paginationKey - The key used for pagination. Undefined if there are no more pages.
 *
...
const extractCollection = queryResult => {
	...
	try {
		if (queryResult['LastEvaluatedKey'] != undefined) {
			paginationKey = btoa(JSON.stringify(queryResult['LastEvaluatedKey']));
		}
	} catch (err) {
		console.error('Error parsing the pagination key');
	}
	return {
		count,
...
const parseCollection = (queryResult, itemMapper) => {
	const { count, data, paginationKey } = extractCollection(queryResult);
	return {
		count,
...
const paginationKeyToDynamo = key => {
	if (key == undefined) {
		return undefined;
	}
	try {
 ...
 * @property {string|null} group - The user's group designation.
 * @property {string|null} subscriptionArn - The Amazon Resource Name (ARN) for the user's subscription.
 */
 ...
 */
export const createUser = async (email, subscriptionArn = null) => {
...
/**
 * @param {string|undefined} paginationKey - The key returned in the last response's paginationKey, if any.
 * @returns {Promise<Collection<User>|string>} In case of failure, an error message will be returned.
 */
export const getAllUsers = async paginationKey => {
	try {
		const res = await client.send(
			new QueryCommand({
				TableName: TABLE_NAME,
				...
				ExclusiveStartKey: paginationKeyToDynamo(paginationKey),
			})
		);
 ...
 * @property {string[]} members - The emails of the group members.
...
export const createGroup = async (groupName, topicArn) => {
	...
	const existingGroup = await findGroup(groupName);
 ...
 */
export const getGroups = async paginationKey => {
	try {
		const res = await client.send(
			new QueryCommand({
				TableName: TABLE_NAME,
 ...
 * @returns {Promise<Collection<Report>|null>}
 */
export const getAllGroupReports = async (groupname, paginationKey) => {
	try {
		const res = await client.send(
			new QueryCommand({
				TableName: TABLE_NAME,
 ...
 * @returns {Promise<StatusResponse>}
 */
export const deleteImage = async (imageId, groupname, username) => {
	try {
		const res = await client.send(
			new DeleteItemCommand({
				...
				Key: {
					[PK]: strToDynamo(imagePK(imageId)),
					[SK]: strToDynamo(imageSK(groupname, username)),
				},
			})
		);
