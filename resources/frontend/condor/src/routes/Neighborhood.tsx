...
import { Group, getGroups, addMemberToGroup, addUser } from "../api/api.ts";
...
function Neighborhood() {
  const { neighborhood, setNeighborhood, email } = userStore();
  ...
  const handleOnClick = async () => {
    ...
    try {
      const add = await addUser(email, neighborhoodState);
      console.log("Processed response from create user:", add);
      const addMember = await addMemberToGroup(neighborhoodState, email);
      console.log("Processed response from add member to group:", addMember);
    } catch (e) {
      console.log("Error while adding user to group:", e);
    }
